...
namespace CBZ.ContactApp.Test.Controllers
{
    public class ReportStatesControllersTests:IClassFixture<DbContextFixture>
    {
        ...
        public void Controller_Post_Should_Be_OkResult()
        {
            ...
            var repository = new ReportStateRepository(_fixture.context);
            var controller = new ReportStatesController(_fixture.context, _logger, repository);
            ActionResult<ReportState> result = controller.Post(ReportStateEntityTypeConfiguration.ReportStateSeed.ElementAt(0));
            result.Result.Should().BeOfType<OkObjectResult>();
        }
        ...
        public void Controller_Put_Should_Be_BadRequest()
        {
            ...
            var repository = new ReportStateRepository(_fixture.context);
            var controller = new ReportStatesController(_fixture.context, _logger, repository);
            ActionResult<ReportState> result = controller.Put(ReportStateEntityTypeConfiguration.ReportStateSeed.ElementAt(2));
            result.Result.Should().BeOfType<BadRequestResult>();
        }
